...
def replace_color_with_red(image_path, target_colors, threshold=30):
    ...
    return num_similar_pixels, total_pixels, result_image
...
#3
temp = [(20/255, 78/255, 26/255), (55/255, 102/255, 47/255), (26/255, 72/255, 28/255)]
...
n = input()
image = cv2.imread(f"{n}.png", cv2.IMREAD_UNCHANGED)
image = resize_image(image, 5)
cv2.imwrite(f"{n}_resize.png", image)

threshold = 30
num_similar, total, r_image = replace_color_with_red(f'{n}_resize.png', target_colours, threshold)
cv2.imwrite(f"{n}_result.png", r_image)
